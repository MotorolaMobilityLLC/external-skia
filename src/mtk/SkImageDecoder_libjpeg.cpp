...
#include "include/mtk/SkScaledBitmapSampler.h"
...
public:
    ...
    bool initializeInfoAndReadHeader() {
        ...
        jpeg_std_oal_ALPHA((j_common_ptr_ALPHA)&fCInfo, &fOal);
        fOal.queryHAL = Codec_queryHAL;
...
public:
    SkJPEGImageDecoder(): fInfoDump(false) {
        fImageIndex = nullptr;
        fImageWidth = 0;
        fImageHeight = 0;
...
#endif
    SkBufMalloc *fBitstreamBuf = nullptr;
    InfoDump fInfoDump;

protected:
    virtual bool onDecodeSubset(SkBitmap* bitmap, BRDAllocator* allocator, const SkIRect& rect, int isampleSize, sk_sp<SkColorSpace> dstColorSpace) override;
};
...
bool SkJPEGImageDecoder::onBuildTileIndex(SkStreamRewindable* stream, int *width, int *height) {
    ...
    dupStream->read(fBitstreamBuf->getAddr(), length) ;
#ifdef MTK_JPEG_HW_REGION_RESIZER
    /* parsing and get ISOSpeedRatings */
    fISOSpeedRatings = getISOSpeedRatings(fBitstreamBuf->getAddr(), length);
    SkDebugf("onBuildTileIndex parsed ISOSpeedRatings %d L:%d!! \n" ,fISOSpeedRatings, __LINE__);
#endif

    SkMemoryStream* mtkPxyStream = new SkMemoryStream(fBitstreamBuf->getAddr(), length, true);
    ...
    int handle = fBitstreamBuf->getIonAllocHnd();
    unsigned int pa = 0;
    ...
    property_get("debug.jpegRegion.input.dump", value, "0");
    mDumpIn = atoi(value);

    if(mDumpIn == 1)
    {
        int ret = sprintf(filepath, "/sdcard/jpeginregion_w%d_h%d_%d.jpg", fImageWidth, fImageHeight, (int)(SkTime::GetNSecs() * 1e-6));
        if (ret < 0)
            SkDebugf("sprintf filepath fail");
        fp = fopen(filepath, "wb");
        if(fp == nullptr)
        {
            SkDebugf("open in dump file error");
        }
        cptr = (unsigned char*)fBitstreamBuf->getAddr();
        if (fp != nullptr)
        {
            if (cptr != nullptr)
            {
...
extern void* allocateBuffer(int ionClientHnd, BufferAllocator *bufferAllocator, ion_user_handle_t *ionAllocHnd, int *bufferFD, size_t size);
extern void freeBuffer(int ionClientHnd, ion_user_handle_t ionAllocHnd, void* bufferAddr, int bufferFD, size_t size);

bool MDPResizer(void* src, int ionClientHnd, BufferAllocator *bufferAllocator, int srcFD, int width, int height, SkScaledBitmapSampler::SrcConfig sc, SkBitmap* bm, SkColorType colorType, int tdsp, void* pPPParam, unsigned int ISOSpeed)
{
    if((nullptr == bm))
    {
       ...
       (colorType == kRGB_565_SkColorType))
    {
        ...
        SkDebugf("MDPResizer allocateBuffer src:(%d), dst:(%d, %d, %d, %d, %p)",
        ...
        if (!ret.isOk())
        {
            ...
            {
                freeBuffer(ionClientHnd, ionAllocHnd, dstBuffer, dstFD, skBitmapSize_MTK);
            }
...
bool SkJPEGImageDecoder::onDecodeSubset(SkBitmap* bm, BRDAllocator* allocator, const SkIRect& region, int isampleSize, sk_sp<SkColorSpace> dstColorSpace) {
    ...
    if(fImageIndex->mtkStream){
        ...
        // Init decoder to image decode mode
        {
            jpeg_std_oal_ALPHA((j_common_ptr_ALPHA)cinfo, &oal);
            oal.queryHAL = Codec_queryHAL;
    ...
    if(!fFirstTileDone || fUseHWResizer)
    {
        ...
        rectHWResz.setLTRB((region.left() >= 40)? region.left() - 40: region.left(),
        ...
        rect.setLTRB(0,0,fImageWidth, fImageHeight);
    ...
    property_get("debug.jpegRegion.output.dump", value, "0");
    mDumpOut = atoi(value);

    if(mDumpOut == 1)
    {
        int ret = sprintf(filepath, "/sdcard/jpegoutregion_w%d_h%d_%d_%d.rgb", width, height, sc, (int)(SkTime::GetNSecs() * 1e-6));
        if (ret < 0)
            SkDebugf("sprintf filepath fail");
        fp = fopen(filepath, "wb");
        if(fp == nullptr)
        {
            SkDebugf("open out dump file error");
            mDumpOut = 1;
        }
    }
#endif
    ...
    if(fInfoDump.enable())
    {
        fInfoDump.add(WIDTH, cinfo->original_image_width);
        fInfoDump.add(HEIGHT, cinfo->image_height);
        fInfoDump.add(PROGRESSIVE, cinfo->progressive_mode);
        fInfoDump.add(TILE_WIDTH, rect.width());
        fInfoDump.add(TILE_HEIGHT, rect.height());
        fInfoDump.add(COLOR_TYPE, colorType);
        fInfoDump.add(COLOR_SPACE, cinfo->out_color_space);
        fInfoDump.add(SAMPLE_SIZE, cinfo->scale_denom);
        fInfoDump.commit();
    }
...
if(!fFirstTileDone || fUseHWResizer)
{
    ...
    #ifdef MTK_SKIA_USE_ION
    SkBufMalloc srcAllocator(fIonClientHnd, fBufferAllocator);
    ...
    while (row_total_count < height) {
        ...
        if (0 == row_count) {
            if (height - 1 == row_total_count)
                SkDebugf("jpeg_read_tile_scanline_ALPHA return 0, set buffer to black when incomplete image");
            memset((void*)rowptr, 0, bpr);
            row_count = 1;
        }
    ...
    {
        #ifdef MTK_SKIA_USE_ION
        result = MDPResizer(hwBuffer, fIonClientHnd, fBufferAllocator, srcAllocator.getFD(), width, height, sc, &bitmap, colorType, enTdshp, nullptr, fISOSpeedRatings);
        #else
        result = MDPResizer(hwBuffer, 0, -1, -1, width, height, sc, &bitmap, colorType, enTdshp, nullptr, fISOSpeedRatings);
        #endif
    ...
    {
        {
            SkAutoMutexExclusive ac(gAutoTileResizeMutex);
    ...
    for (int y = 0;; y++) {
        ...
        if (0 == row_count) {
            if(bitmap.height() - 1 == y)
                SkDebugf("jpeg_read_tile_scanline_ALPHA return 0, set buffer to black when incomplete image");
            memset((void*)rowptr, 0, width * srcBytesPerPixel);
            row_count = 1;
        }
        ...
        {
            cptr = (unsigned char*)rowptr;
            for (int i = 0; i < width * srcBytesPerPixel; i++)
            {
                fprintf(fp, "%c", *cptr);
                cptr++;
            }
