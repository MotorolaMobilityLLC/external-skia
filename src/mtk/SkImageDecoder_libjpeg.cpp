...
static void overwrite_mem_buffer_size(jpeg_decompress_struct_MTK* cinfo) {
#ifdef SK_BUILD_FOR_ANDROID
...
static void do_nothing_emit_message(jpeg_common_struct_MTK*, int) {
    /* do nothing */
}
static void do_nothing_output_message(j_common_ptr_MTK) {
    /* do nothing */
}

static void initialize_info(jpeg_decompress_struct_MTK* cinfo, skjpeg_source_mgr_MTK* src_mgr) {
...
public:
    ...
    void destroyInfo() {
        ...
        // function to prevent potential infinite loop.
        fInfoInitialized = false;
        jpeg_destroy_decompress_MTK(&fCInfo);
        SkDEBUGCODE(fReadHeaderSucceeded = false;)
    }

    /**
     *  Initialize the cinfo struct.
     *  Calls jpeg_create_decompress_MTK, makes customizations, and
    ...
    bool initializeInfoAndReadHeader() {
        ...
        const bool success = (JPEG_HEADER_OK_MTK == jpeg_read_header_MTK(&fCInfo, true));
        SkDEBUGCODE(fReadHeaderSucceeded = success;)
        return success;
    }

    jpeg_decompress_struct_MTK* cinfo() { return &fCInfo; }

    huffman_index_MTK* huffmanIndex() { return &fHuffmanIndex; }
    ...
    bool buildHuffmanIndex() {
        ...
        jpeg_create_huffman_index_MTK(&fCInfo, &fHuffmanIndex);
        SkASSERT(1 == fCInfo.scale_num && 1 == fCInfo.scale_denom);
        fHuffmanCreated = jpeg_build_huffman_index_MTK(&fCInfo, &fHuffmanIndex);
    ...
    bool startTileDecompress() {
        ...
        SkASSERT(fReadHeaderSucceeded);
        SkASSERT(!fDecompressStarted);
        if (jpeg_start_tile_decompress_MTK(&fCInfo)) {
...
private:
    ...
    jpeg_decompress_struct_MTK fCInfo;
    huffman_index_MTK fHuffmanIndex;
...
private:
     ...
     */
    SkColorType getBitmapColorType(jpeg_decompress_struct_MTK*);
...
public:
    ...
    ~JPEGAutoClean() {
        if (cinfo_ptr) {
            jpeg_destroy_decompress_MTK(cinfo_ptr);
        }
    }
    void set(jpeg_decompress_struct_MTK* info) {
        cinfo_ptr = info;
    }
private:
    jpeg_decompress_struct_MTK* cinfo_ptr;
};
...
static int recompute_sampleSize(int sampleSize,
                                const jpeg_decompress_struct_MTK& cinfo) {
    return sampleSize * cinfo.output_width / cinfo.image_width;
}

static bool valid_output_dimensions(const jpeg_decompress_struct_MTK& cinfo) {
...
static bool skip_src_rows(jpeg_decompress_struct_MTK* cinfo, void* buffer, int count) {
    for (int i = 0; i < count; i++) {
        JSAMPLE_MTK* rowptr = (JSAMPLE_MTK*)buffer;
        int row_count = jpeg_read_scanlines_MTK(cinfo, &rowptr, 1);
        if (1 != row_count) {
            return false;
        }
    }
...
#ifdef SK_BUILD_FOR_ANDROID
static bool skip_src_rows_tile(jpeg_decompress_struct_MTK* cinfo,
                               huffman_index_MTK *index, void* buffer, int count) {
    for (int i = 0; i < count; i++) {
        JSAMPLE_MTK* rowptr = (JSAMPLE_MTK*)buffer;
        int row_count = jpeg_read_tile_scanline_MTK(cinfo, index, &rowptr);
        if (1 != row_count) {
            return false;
        }
    }
...
static void print_jpeg_decoder_errors(const jpeg_decompress_struct_MTK& cinfo,
                         int width, int height, const char caller[]) {
    if (!(c_suppressJPEGImageDecoderErrors)) {
        char buffer[JMSG_LENGTH_MAX_MTK];
        cinfo.err->format_message((const j_common_ptr_MTK)&cinfo, buffer);
        SkDebugf("libjpeg error %d <%s> from %s [%d %d]\n",
                 cinfo.err->msg_code, buffer, caller, width, height);
    }
}

static bool return_false(const jpeg_decompress_struct_MTK& cinfo,
                         const char caller[]) {
...
static SkImageDecoder::Result return_failure(const jpeg_decompress_struct_MTK& cinfo,
                                             const SkBitmap& bm, const char caller[]) {
    print_jpeg_decoder_errors(cinfo, bm.width(), bm.height(), caller);
 ...
 */
static void set_error_mgr(jpeg_decompress_struct_MTK* cinfo, skjpeg_error_mgr_MTK* errorManager) {
    SkASSERT(cinfo != nullptr);
    SkASSERT(errorManager != nullptr);
    cinfo->err = jpeg_std_error_MTK(errorManager);
    errorManager->error_exit = skjpeg_error_exit_MTK;
}
 ...
 *  resulting bitmap.
 */
static void turn_off_visual_optimizations(jpeg_decompress_struct_MTK* cinfo) {
    SkASSERT(cinfo != nullptr);
 ...
 */
static void set_dct_method(const SkImageDecoder& decoder, jpeg_decompress_struct_MTK* cinfo) {
    SkASSERT(cinfo != nullptr);
...
SkColorType SkJPEGImageDecoder::getBitmapColorType(jpeg_decompress_struct_MTK* cinfo) {
    SkASSERT(cinfo != nullptr);

    SrcDepth srcDepth = k32Bit_SrcDepth;
    if (JCS_GRAYSCALE_MTK == cinfo->jpeg_color_space) {
    ...
    switch (colorType) {
        case kAlpha_8_SkColorType:
            ...
            // values.
            if (cinfo->jpeg_color_space != JCS_GRAYSCALE_MTK) {
    ...
    switch (cinfo->jpeg_color_space) {
        ...
        case JCS_YCCK_MTK:
            ...
            cinfo->out_color_space = JCS_CMYK_MTK;
            break;
        case JCS_GRAYSCALE_MTK:
            if (kAlpha_8_SkColorType == colorType) {
                cinfo->out_color_space = JCS_GRAYSCALE_MTK;
                break;
            }
 ...
 */
static void adjust_out_color_space_and_dither(jpeg_decompress_struct_MTK* cinfo,
                                              SkColorType colorType,
                                              const SkImageDecoder& decoder) {
    SkASSERT(cinfo != nullptr);
#ifdef ANDROID_RGB
    cinfo->dither_mode = JDITHER_NONE_MTK;
    if (JCS_CMYK_MTK == cinfo->out_color_space) {
        return;
    }
    switch (colorType) {
        case kN32_SkColorType:
 ...
 */
static bool get_src_config(const jpeg_decompress_struct_MTK& cinfo,
    ...
    if (JCS_CMYK_MTK == cinfo.out_color_space) {
        ...
        *srcBytesPerPixel = 4;
    } else if (3 == cinfo.out_color_components && JCS_RGB_MTK == cinfo.out_color_space) {
        ...
        *srcBytesPerPixel = 2;
               ...
               JCS_GRAYSCALE_MTK == cinfo.out_color_space) {
...
SkImageDecoder::Result SkJPEGImageDecoder::onDecode(SkStream* stream, SkBitmap* bm, Mode mode) {
    ...
    */
    if (!jpeg_start_decompress_MTK((jpeg_decompress_struct_MTK*)(&cinfo))) {
    ...
    */
    ...
    for (int y = 0;; y++) {
        JSAMPLE_MTK* rowptr = (JSAMPLE_MTK*)srcRow;
        int row_count = jpeg_read_scanlines_MTK(&cinfo, &rowptr, 1);
        if (0 == row_count) {
            // if row_count == 0, then we didn't get a scanline,
            // so return early.  We will return a partial image.
...
static SkISize compute_yuv_size(const jpeg_decompress_struct_MTK& info, int component,
                                SizeType sizeType) {
    if (sizeType == kSizeForMemoryAllocation_SizeType) {
        return SkISize::Make(info.cur_comp_info[component]->width_in_blocks * DCTSIZE_MTK,
                             info.cur_comp_info[component]->height_in_blocks * DCTSIZE_MTK);
    }
    return SkISize::Make(info.cur_comp_info[component]->downsampled_width,
                         info.cur_comp_info[component]->downsampled_height);
}

static bool appears_to_be_yuv(const jpeg_decompress_struct_MTK& info) {
    return (info.jpeg_color_space == JCS_YCbCr_MTK)
        && (DCTSIZE_MTK == 8)
...
static void update_components_sizes(const jpeg_decompress_struct_MTK& cinfo, SkISize componentSizes[3],
                                    SizeType sizeType) {
...
static bool output_raw_data(jpeg_decompress_struct_MTK& cinfo, void* planes[3], size_t rowBytes[3]) {
    SkASSERT(appears_to_be_yuv(cinfo));
    ...
    JSAMPARRAY_MTK bufferraw[3];
    JSAMPROW_MTK bufferraw2[32];
    ...
    JSAMPROW_MTK outputY = static_cast<JSAMPROW_MTK>(planes[0]);
    JSAMPROW_MTK outputU = static_cast<JSAMPROW_MTK>(planes[1]);
    JSAMPROW_MTK outputV = static_cast<JSAMPROW_MTK>(planes[2]);
    size_t rowBytesY = rowBytes[0];
    size_t rowBytesU = rowBytes[1];
    size_t rowBytesV = rowBytes[2];

    int yScanlinesToRead = DCTSIZE_MTK * v;
    SkAutoTMalloc<uint8_t> lastRowStorage(rowBytesY * 4);
    JSAMPROW_MTK yLastRow = (JSAMPROW_MTK)lastRowStorage.get();
    JSAMPROW_MTK uLastRow = yLastRow + rowBytesY;
    JSAMPROW_MTK vLastRow = uLastRow + rowBytesY;
    JSAMPROW_MTK dummyRow = vLastRow + rowBytesY;
    ...
    while (cinfo.output_scanline < cinfo.output_height) {
        ...
        JDIMENSION_MTK scanlinesRead = jpeg_read_raw_data_MTK(&cinfo, bufferraw, yScanlinesToRead);
...
bool SkJPEGImageDecoder::onDecodeYUV8Planes(SkStream* stream, SkISize componentSizes[3],
    ...
    cinfo.out_color_space = JCS_YCbCr_MTK;
    cinfo.raw_data_out = TRUE_MTK;
...
#ifdef SK_BUILD_FOR_ANDROID
static bool getEncodedColor(jpeg_decompress_struct_MTK* cinfo, SkEncodedInfo::Color* outColor) {
    switch (cinfo->jpeg_color_space) {
        case JCS_GRAYSCALE_MTK:
...
//    stream->rewind();
    ...
    jpeg_decompress_struct_MTK* cinfo = imageIndex->cinfo();
...
bool SkJPEGImageDecoder::onDecodeSubset(SkBitmap* bm, const SkIRect& region, int isampleSize, void* pParam) {
    ...
    if(fImageIndex->mtkStream){
        ...
        cinfo = (jpeg_decompress_struct_MTK *)malloc(sizeof(struct jpeg_decompress_struct_MTK));
        memset(cinfo, 0, sizeof(struct jpeg_decompress_struct_MTK));
        auto_clean_cinfo.set(cinfo);

        cinfo->src = (jpeg_source_mgr_MTK *)malloc(sizeof(struct jpeg_source_mgr_MTK));
        memset(cinfo->src, 0, sizeof(struct jpeg_source_mgr_MTK));
        ...
        //if (!localImageIndex->initializeInfoAndReadHeader()) 
        {
            ...
            const bool success = (JPEG_HEADER_OK_MTK == jpeg_read_header_MTK(cinfo, true));
            if(!success){
        ...
        if (!jpeg_start_tile_decompress_MTK(cinfo)) {
...
#else
    jpeg_decompress_struct_MTK* cinfo = fImageIndex->cinfo();
#endif
    ...
    jpeg_init_read_tile_scanline_MTK(cinfo, fImageIndex->huffmanIndex(),
                                 &startX, &startY, &width, &height);
    int skiaSampleSize = recompute_sampleSize(requestedSampleSize, *cinfo);
    int actualSampleSize = skiaSampleSize * (DCTSIZE_MTK / cinfo->min_DCT_scaled_size);
    ...
    */
         ...
         (kRGB_565_SkColorType == colorType && cinfo->out_color_space == JCS_RGB_565)))
    {
        ...
        while (rowTotalCount < height) {
            int rowCount = jpeg_read_tile_scanline_MTK(cinfo,
...
if(!fFirstTileDone || fUseHWResizer)
{
    ...
    JSAMPLE_MTK* rowptr = (JSAMPLE_MTK*)hwBuffer;
    
    //SkDebugf("MTR_JPEG: jpeg_read_tile_scanline 2+ , time = %f , L:%d!!\n",now_ms() - g_mt_start  , __LINE__);
    
    while (row_total_count < height) {
        int row_count = jpeg_read_tile_scanline_MTK(cinfo, fImageIndex->huffmanIndex(), &rowptr);
